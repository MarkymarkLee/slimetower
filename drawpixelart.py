...
while run:
    ...
    if P <= x and x <= N*P and P <= y and y <= N*P:
    ...
    if (N+1)*P+10 <= x and x <= (N+1)*P+40 and 10 <= y and y <= 10+BH:
        cc = 0

    if (N+1)*P+10 <= x and x <= (N+1)*P+40 and 20+BH <= y and y <= 20+2*BH:
        cc = 1

    if (N+1)*P+10 <= x and x <= (N+1)*P+40 and 30+2*BH <= y and y <= 30+3*BH:
        time.sleep(0.2)
        if not N%16==0:
        ...
        for k in range(int(N/16)):
            ...
            #print(f"do Sys.wait(50);")
        


    if (N+1)*P+10 <= x and x <= (N+1)*P+10+30 and 40+3*BH+(BH//2) <= y and y <= 40+3*BH+(BH//2)+BH/4:
        #print(map)
        map = [[0 for i in range(N)] for i in range(N)]
        time.sleep(1)
